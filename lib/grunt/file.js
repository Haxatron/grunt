...
file.write = function(filepath, contents, options) {
  ...
  try {
    ...
    // If path is a symlink, delete it first
    if (file.isLink(filepath)) {
...
file._copy = function(srcpath, destpath, options) {
  ...
  if ( !options.followSymlink && file.isLink(srcpath)) {
    // If destpath is a symlink, delete it first
    if (file.isLink(destpath)) {
      fs.unlinkSync(destpath);
    }
    fs.symlinkSync(fs.readlinkSync(srcpath), destpath, win32 ? 'junction' : null);
    return;
  }
