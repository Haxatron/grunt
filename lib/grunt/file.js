...
file._copy = function(srcpath, destpath, options) {
  ...
  // Copy symlinks, and overwrite symlinks by using renaming method (to prevent race conditions)
  if ( !options.followSymlink && file.isLink(srcpath)) {
    var destpathTmp = destpath + '_gruntjs_' + crypto.randomBytes(16).readUInt32LE(0);
    fs.symlinkSync(fs.readlinkSync(srcpath), destpathTmp, win32 ? 'junction' : null);
  ...
  if (contents === false) {
    ...
    // Overwrite symlinks by using renaming method (to prevent race condition)
    if ( !options.followSymlink && file.isLink(srcpath) ) {
      var destpathTmp = destpath + '_gruntjs_' + crypto.randomBytes(16).readUInt32LE(0);
      file.write(destpathTmp, contents, readWriteOptions);
      fs.renameSync(destpathTmp, destpath);
    } else {
